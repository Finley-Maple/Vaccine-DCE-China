...
write.table(model, file = "database.csv")
```
...
VaccineMode <- mlogit.data(full_dat,choice="choice",shape="long",
...
#### 2. Data loading                                   ####
# ####################################################### #
database <- read.table(file = "database.csv")
...
apollo_draws = list(
  ...
  interNDraws    = 250,
  interUnifDraws = c(),
  interNormDraws = paste0("draws_", c("price", "risk2", 
                                      "risk3", "duration2", 
                                      "duration3", "efficacy2", 
                                      "efficacy3", "oral", 
                                      "doses2", "doses3", 
...
apollo_randCoeff = function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["b_doses2"]] = b_doses2_mu + b_doses2_sigma*draws_doses2
  randcoeff[["b_doses3"]]     = b_doses3_mu     + b_doses3_sigma    *draws_doses3 
...
apollo_probabilities <- function(apollo_beta, apollo_inputs, functionality = "estimate"){
  ...
  V[['alt1']] = asc + b_price*price.1 + b_risk2*risk2.1 + b_risk3*risk3.1 + 
    ...
    b_efficacy3*efficacy3.1 + b_oral*admin.1 + b_doses2*doses2.1 + 
    b_doses3*doses3.1 + b_imported*origin.1
  V[['alt2']] = asc + b_price*price.2 + b_risk2*risk2.2 + b_risk3*risk3.2 + 
    ...
    b_efficacy3*efficacy3.2 + b_oral*admin.2 + b_doses2*doses2.2 + 
    b_doses3*doses3.2 + b_imported*origin.2
  V[['alt3']] = 0
...
# ####################################################### #
#### 7. Model estimation and reporting                 ####
# ####################################################### #
...
for (i in c("b_risk2_mu", "b_risk3_mu", "b_duration2_mu", "b_duration3_mu", 
            "b_oral_mu", "b_doses2_mu", "b_doses3_mu", "b_imported_mu")) {
  deltaMethod_settings <- list(operation = "ratio", parName1 = i, parName2 = "b_price_mu",
                               multPar1 = -1)
...
```{r appendix conditional logit model: use apollo package to analyze}
# use apollo package to analyze
# ####################################################### #
#### 1. Definition of core settings                        
# ####################################################### #
